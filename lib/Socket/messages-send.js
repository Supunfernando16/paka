...
const makeMessagesSocket = (config) => {
    ...
    const ayun = new lord(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = ayun.detectType(content);
            ...
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await ayun.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await ayun.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await ayun.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await ayun.handleAlbum(content, jid, quoted)
                    case 'EVENT':
                        return await ayun.handleEvent(content, jid, quoted)
                    case 'POLL_RESULT':
                        return await ayun.handlePollResult(content, jid, quoted)
                    case 'GROUP_STORY':
                        return await ayun.handleGroupStory(content, jid, quoted)
                }
            }
